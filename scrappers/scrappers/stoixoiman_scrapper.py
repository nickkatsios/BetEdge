...
class Stoiximan_scrapper(Scrapper_interface):
    ...
    def close_popups_and_cookies(self):
        ...
        try:
           # locate close button of popup ad and close
            ...
            # Accept cookies
            accept_cookes_btn = WebDriverWait(self.driver, timeout=0.5).until(
    ...
    def wait_for_page_to_load(self):
        ...
        WebDriverWait(self.driver, timeout=3).until(
            EC.element_to_be_clickable((By.CLASS_NAME , "selections__selection")))
    ...
    def get_event_league(self):
        ...
        """
        bar_items = self.driver.find_elements(By.CLASS_NAME , "breadcrumbs-container__list__item")
        league = bar_items[-2].text
        return league

    def get_event_teams(self):
        """ Returns the 2 teams of the event"""
        teams = self.driver.find_elements(By.CLASS_NAME , "participant")
        team1 = teams[0].text
        team2 = teams[1].text
        return team1, team2
    ...
    def get_event_datetime(self):
        ...
        """
        time_str = self.driver.find_element(By.CLASS_NAME , "event-details__info__top__datetime").text
        # Sanitization
        ds = Date_standardizer()
        datetime_obj = ds.standardize_date(time_str, "stoiximan")
        return datetime_obj
    ...
    def get_market_odds(self, market, market_name):
        ...
        """
        options = market.find_elements(By.CLASS_NAME , "selections__selection")
        option_titles = []
        odds = []
        for option in options:
            option_title = option.find_element(By.CLASS_NAME , "selections__selection__title").text
            # Sanitization remove \n
            option_title = option_title.replace("\n" , " ")
            # No need for option_title standardization because stoiximan has correct titles
            option_titles.append(option_title)
            option_odds = option.find_element(By.CLASS_NAME , "selections__selection__odd").text
            # Sanitization parse to float
        ...
        if self.allowed_markets[market_name] == 1:
            ...
            option_titles = ots.standardize_event_winner_titles(self.sport)
    ...
    def get_markets(self):
        ...
        """
        markets = self.driver.find_elements(By.CLASS_NAME , "markets__market")
        market_names = []
        for market in markets:
            name = market.find_element(By.CLASS_NAME , "markets__market__header__title").text
    ...
    def run_scrapper(self):
        ...
        """
        self.logger.info(f"Running Stoiximan scrapper in {self.driver.name.upper()} browser")
        for index, url in enumerate(self.urls):
            self.logger.info(f"Navigating to URL: {url}: {index} out of {len(self.urls)} ")
            try:
                # Navigate to url
                self.driver.get(url)
                # Close popups and accept cookies
                if index == 0:
                ...
                for market, market_name in zip(markets , market_names):
                    ...
                    if self.allowed_markets.get(market_name) is not None:
                        ...
                        # Look up in configs dictionary to get market id
